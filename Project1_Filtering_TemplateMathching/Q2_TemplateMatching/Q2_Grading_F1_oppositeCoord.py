    ...
    for coord_input in input_ls:
        for coord_ground in groundtruth_ls:
            ...
            if distance(coord_ground[0], coord_ground[1],
                        int(coord_input[1]) + 0.5 * int(tp_h), int(coord_input[0]) + 0.5 * int(tp_w)) < tolerance:
...
def question2_grader(groundtruth_path, json_path):
    ...
    f1 = f1_score(groundtruth_ls=gorundtruth_ls, input_ls=input_ls, tp_h=tmp_ls[1], tp_w=tmp_ls[0])
    return f1
...
def main():
    ...
    csv_file = 'question2_score_new_2Run_2.csv'
